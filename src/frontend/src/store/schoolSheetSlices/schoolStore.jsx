...
export const getStaffcustomers = createAsyncThunk('schoolSheet/staffcustomers', async () => {
	const staffcustomers = await axiosInstance.get('/staff');
	const { data } = staffcustomers;
	const { status, payload } = data;
	if (status) return payload;
});
...
export const schoolSheetSlices = createSlice({
	...
	extraReducers: {
		...
		[getStaffcustomers.pending]: (state) => {
			state.loading.staffcustomers = true;
		},
		[getStaffcustomers.fulfilled]: (state, action) => {
			state.loading.staffcustomers = false;
			state.staffcustomers = action.payload;
		},
