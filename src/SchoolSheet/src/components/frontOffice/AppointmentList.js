...
function AppointmentList() {
	...
	return (
        <div className="bg-white p-2">
			{/* edit popup start */}
		{editData ? <EditAppointment closeEditData={closeEditData} /> : null}
			{/* edit popup end */}



			{/* show user popup start */}
			{showData ? <ShowAppointment closeShowData={closeShowData} /> : null}
			{/* show user popup end */}
		<table className="mt-4 w-full table-auto">
			<thead style={{ backgroundColor: "#0d6dfd10" }}>
				...
				<th className="p-2 text-primary text-sm text-left">Another Contact</th>
				<th className="p-2 text-primary text-sm text-left">Email</th>
				<th className="p-2 text-primary text-sm text-left">Next of Kin</th>
				<th className="p-2 text-primary text-sm text-left">NOK Phone</th>
				...
				{patients.map((patient) => {
					return (
						...
						>
							...
							<td className="text-xs p-3 text-gray5">{patient.contacts[1]}</td>
							<td className="text-xs p-3 text-gray5">{patient.email}</td>
							<td className="text-xs p-3 text-gray5">{patient.noks[0].name}</td>
							<td className="text-xs p-3 text-gray5">
								{patient.noks[0].phone}
							</td>
							<td className="text-xs p-3 text-gray5 flex justify-between">
								<MdDeleteOutline className="text-red w-4 h-4" />
								<BsPencilSquare onClick={() => openEditData(patient)} className="text-warning h-4 w-4" />
								<BsEye onClick={() => openShowData(patient)} className="text-primary h-4 w-4" />
							</td>
						</tr>
					);
				})}
