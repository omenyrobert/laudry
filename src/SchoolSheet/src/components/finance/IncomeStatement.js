...
function IncomeStatement() {
	...
	useEffect(() => {
		if (accounts) {
			...
			const expSum = groupedAccounts.expenses.reduce((total, exp) => total + exp.amount, 0);
			const incSum = groupedAccounts.incomes.reduce((total, inc) => total + inc.amount, 0);
			setSumInc(incSum);
			setSumExp(expSum);
			...
			<div className="bg-white p-3 h-[82vh] overflow-y-auto mt-2 border border-gray2 shadow rounded">
				...
				<div  id="ledger-table">
					...
					<div className="flex bg-primary3 text-primary border border-primary3">
						...
						<div className="w-2/12 border border-primary3 p-3">{sumInc}</div>
					</div>
					...
					<div className="flex bg-primary3 text-primary border border-primary3">
						...
						<div className="w-2/12 border border-primary3 p-3">{sumExp}</div>
					</div>
					<div className="flex bg-primary text-white mt-5">
						<div className="border border-gray1 p-3  w-10/12">Net Profit</div>

						<div className="border border-gray1 p-3 w-2/12">{sumInc - sumExp}</div>
					</div>
				</div>
				<br />
			</div>
