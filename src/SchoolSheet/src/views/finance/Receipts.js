	...
	const deletereceipt = (receiptItem) => {
		Swal.fire({
			...
			if (result.isConfirmed) {
				...
				axiosInstance.delete(`/transactions/${receiptItem.transactionId}`)
					.then((response) => {
			...
			<div className="w-full h-[80vh]">
				...
				<table className="mt-5 w-[98%] table-auto">
					<thead style={{ backgroundColor: "#0d6dfd10" }}>
						...
						<th className="p-2 text-primary text-sm text-left">receipt Type</th>
						...
						{receiptsData.map((receiptItem) => {
							return (
								...
								>
									...
									<td className="text-xs p-3 text-gray5">{receiptItem.receivedBy}</td>
									<td className="text-xs p-3 text-gray5">
									...
									<td className="text-xs p-3 text-gray5 flex">
										...
										<BsPencilSquare
											...
											onClick={() => {
												navigate(`/addTransaction?transactionType=receipt&action=edit&transactionId=${receiptItem.transactionId}`)
											}}
										/>
									</td>
								</tr>
