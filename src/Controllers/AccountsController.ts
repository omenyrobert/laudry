...
export const payAccountController = async (req: Request, res: Response) => {
  try {
    ...
    if (!amount) {
      return res
        .json(customPayloadResponse(false, "Amount Required"))
        .status(200)
        .end();
    }
    if (!date) {
    ...
    return res
      .json(customPayloadResponse(true, "Account Paid"))
      .status(200)
      .end();
  } catch (error) {
    console.log(error);
...
export const handleUpdatePayment = async (req: Request, res: Response) => {
  try {
    ...
    await updatePayment(id, parseInt(amount), date);
    return res
      .json(customPayloadResponse(true, "Payment updated"))
      .status(200)
      .end();
  } catch (error) {
    console.log(error);
...
export const handleDeletePayment = async (req: Request, res: Response) => {
  try {
    ...
    return res
      .json(customPayloadResponse(true, "payment deleted"))
      .status(200)
      .end();
  } catch (error) {
    console.log(error);
    return res.json(customPayloadResponse(false, error)).status(500).end();
  }
};
